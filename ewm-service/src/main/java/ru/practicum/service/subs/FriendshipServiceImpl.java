...
import ru.practicum.model.QFriendship;
...
public class FriendshipServiceImpl implements FriendshipService {
    ...
    public FriendshipDto requestFriendship(long followerId, long userId) {
        ...
        final Friendship friendshipRequest = Friendship.builder()
                .follower(follower)
                .friend(friend)
                .state(friend.isAutoSubscribe() ? FriendshipState.APPROVED : FriendshipState.PENDING)
    ...
    private List<FriendshipShortDto> getRequestByFilter(String filter, QUser user, long userId) {
        final List<Predicate> eq = ("ALL".equalsIgnoreCase(filter))
                ...
                : List.of(user.id.eq(userId), QFriendship.friendship.state.eq(FriendshipState.from(filter)));
    ...
    private void throwWhenFriendshipExist(long userId, long friendId) {
        if (friendshipRepository.existsByFollowerIdAndFriendIdAndStateNot(userId, friendId, FriendshipState.REJECTED)) {
